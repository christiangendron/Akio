...
export default function PostInteraction(props: PostProps) {
  ...
  return (
    <View className='gap-1 flex-row p-2'>
      <TouchableOpacity className='flex-row flex space-x-1 bg-gray-300 rounded-lg p-2' onPress={() => navigation.push('Community', { name: props.name, id: props.community_id })}>
        <Text>{props.name}</Text>
      </TouchableOpacity>
      <TouchableOpacity className='flex-row flex space-x-1 bg-gray-300 rounded-lg p-2' onPress={() => navigation.push('Details', { ...props })}>
