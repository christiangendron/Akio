...
export default function PostsList(props: PostsListNavigationProps) {
	...
	const renderItem = ({ item }: { item: any }): JSX.Element => {
		return <SwipeableDelete 
			...
			component={<SmallPost key={item.id} {...item} keyToInvalidate={queryKey} />}/>
	};

	const searchComp = <SearchBarComp keyword={keyword} handleChange={setKeyword} handleSubmit={query.refetch} placeholder={`Search...`}/>;
	...
	return (
		<View className='flex flex-1 justify-center items-center bg-background dark:bg-backgroundDark'>
			<CustomFlatList 
				...
				keyToInvalidate={queryKey}
				headerComponent={withSearch ? searchComp : null}
			/>
		</View>
	);
}